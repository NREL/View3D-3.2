...
void CTIdent( R4 t[4][4] )
  {
  IX j;

  memset( (void *)t, 0, 16*sizeof(R4) );
  for( j=0; j<4; j++ )
    t[j][j] = 1.0;
...
void CTScale( const R4 s, R4 t[4][4] )
  {
  IX i,j;
...
void CTShift( const VERTEX3D *s, R4 t[4][4] )
  {
  IX j;
...
void CTRotateX( const R4 cosAngle, const R4 sinAngle, R4 t[4][4] )
  {
  IX j;
...
void CTRotateZ( const R4 cosAngle, const R4 sinAngle, R4 t[4][4] )
  {
  IX j;
...
void CTRotateU( const DIRCOS *u, R4 t[4][4] )
  {
  R4 v;
...
void CT3D( const IX nv, const R4 t[4][4], const VECTOR3D *p, VECTOR3D *q )
  {
  IX n;
 ...
 */
  ...
  R4 z[MAXNV1];  /* Z coordinates */
  R4 a[4][4]; /* coordinates transformation matrix */
  R4 b[4][4]; /* coordinate rotation matrix */
  R4 scale; /* coordinate scaling factor */
  R4 zmax;  /* Z coordinate of highest vertex */
  R4 eps=-1.0e-6f;
  ...
  for( j=1; j<=vfCtrl->nProbObstr; j++,srfOT++ )
    ...
    if( clip )    /* this may never be needed for plane polygons */
      ...
      srfOT->nv = ClipPolygon( 1.0, nv, vs, z, srfOT->v );
      }
    }
...
/*  Dump a vector {A} or array [A] (R4 values) to file _ulog.
