    ...
    for( k=baseSrf[maskSrf[j]]; k; k=baseSrf[k] )
      if( k == Ns )
        ...
        for( n=0; n<nv; n++ )        /* mask may not be behind srfM */
          if( VDOTW( (ps->v[n]), (&srfM->dc) ) > eps ) break;
        if( n==nv ) continue;
    ...
    for( k=baseSrf[maskSrf[j]]; k; k=baseSrf[k] )
      if( k == Ms )
        ...
        for( n=0; n<nv; n++ )        /* mask may not be behind srfN */
          if( VDOTW( (ps->v[n]), (&srfN->dc) ) > eps ) break;
        if( n==nv ) continue;
        possibleObstr[++nPoss] = maskSrf[j];
        ps->MrelS = 1;
        ps->NrelS = -1;
 ...
 */
  {
  R4 xmax, xmin, ymax, ymin, zmax, zmin;  /* limits of box enclosing N & M */
  IX nv;     /* number of vertices */
  IX n;      /* vertex number */
  ...
  for( n=1; n<srfN->nv; n++ )
    {
    if( srfN->v[n].x > xmax ) xmax = srfN->v[n].x;
    if( srfN->v[n].x < xmin ) xmin = srfN->v[n].x;
    if( srfN->v[n].y > ymax ) ymax = srfN->v[n].y;
    if( srfN->v[n].y < ymin ) ymin = srfN->v[n].y;
    if( srfN->v[n].z > zmax ) zmax = srfN->v[n].z;
    if( srfN->v[n].z < zmin ) zmin = srfN->v[n].z;
    }
  for( n=0; n<srfM->nv; n++ )
    {
    if( srfM->v[n].x > xmax ) xmax = srfM->v[n].x;
    if( srfM->v[n].x < xmin ) xmin = srfM->v[n].x;
    if( srfM->v[n].y > ymax ) ymax = srfM->v[n].y;
    if( srfM->v[n].y < ymin ) ymin = srfM->v[n].y;
    if( srfM->v[n].z > zmax ) zmax = srfM->v[n].z;
    if( srfM->v[n].z < zmin ) zmin = srfM->v[n].z;
    }
  ...
  for( nPoss=0,i=1; i<=nPossObstr; i++ )
    ...
    for( n=0; n<nv; n++ )
      if( srf[k].v[n]->x < xmax ) break;
    if( n==nv ) continue;    /* all vertices > xmax; no obstruction */
    for( n=0; n<nv; n++ )
      if( srf[k].v[n]->x > xmin ) break;
    if( n==nv ) continue;    /* all vertices < xmin; no obstruction */

    for( n=0; n<nv; n++ )
      if( srf[k].v[n]->y < ymax ) break;
    if( n==nv ) continue;
    for( n=0; n<nv; n++ )
      if( srf[k].v[n]->y > ymin ) break;
    if( n==nv ) continue;

    for( n=0; n<nv; n++ )
      if( srf[k].v[n]->z < zmax ) break;
    if( n==nv ) continue;
    for( n=0; n<nv; n++ )
      if( srf[k].v[n]->z > zmin ) break;
    if( n==nv ) continue;
...
IX ConeRadiusTest( SRFDAT3D *srf, SRFDATNM *srfN, SRFDATNM *srfM,
  VFCTRL *vfCtrl, IX *possibleObstr, IX nPossObstr, R4 distNM )
/* srf  - data for all surfaces.
 * srfN - data for surface N.
 * srfM - data for surface M.
 * possibleObstr  - list of possible obstructing surfaces (output).
 ...
 */
  ...
  IX mode=0; /* test mode; 0 = cylinder; -1 = cone from srfM;
                             +1 = cone from srfN  */
  R4 radCylndr;  /* radius of cylinder */
  R4 radSmall,   /* radius of smaller surface */
     radLarge;   /* radius of larger surface */
  R4 distSmall,  /* distance from apex of cone to smaller surface */
 ...
 */
  {
  R4 dot, eps; /* dot product and test value */
  ...
  for( nPoss=0,i=1; i<=nPossObstr; i++ )
    ...
    for( n=0; n<nv; n++ )
      if( VDOTW( (srf[k].v[n]), (&srfM->dc) ) > eps ) break;
    if( n==nv ) continue;
#if( DEBUG > 1 )
    ...
    if( infront + behind == 0 ) continue;   /* coplanar surfaces */
    ...
    if( srf[k].NrelS * srf[k].MrelS > 0 ) continue;
 ...
 */
  ...
  for( nPoss=0,i=1; i<=nPossObstr; i++ )
    ...
    for( j=srf[k].nv; j; j-- )
      if( VDOTW( (srf[k].v[j-1]), (&srf[N].dc) ) > eps ) break;
...
IX ClipPolygon( const R4 flag, const IX nVu, VERTEX3D *vPoly, R4 *dist,
  VERTEX3D *vClipPoly )
 ...
 * vClipPoly - vertices of the clipped polygon. */
  ...
  for( j=0; j<nVu; jm1=j++ )
    ...
    if( dist[j] * dist[jm1] < 0.0 )   /* save intercept with plane */
      ...
      R4 h;          /* of clipping plane with neither vertex on the plane. */
...
void SelfObstructionClip( SRFDATNM *srfN )
  ...
  srfN->nv = ClipPolygon( 1.0, srfN->nv, srfN->v, srfN->dist, v );
...
IX SetShape( const IX nv, VERTEX3D *v, R4 *area )
  ...
  if( nv == 3 )
    ...
    *area = Triangle( v+0, v+1, v+2, &c, 0 );
    ...
    R4 edge[4];       /* length^2 of edge vectors */
    R4 area0, area2;
...
IX SelfObstructionTest3D( SRFDAT3D *srf1, SRFDAT3D *srf2, SRFDATNM *srfN )
  ...
  R4 dist;   /* distance in front of plane; save for clipping calc. */
  ...
  if( !infront && !behind ) /* more detailed test: */
    {
    R4 eps=1.0e-5f*srf2->rc; /* test value for "close to plane of SRF1" */
  ...
  if( infront )
    ...
    static const IX size=(3*sizeof(IX)+2*sizeof(R4)+sizeof(DIRCOS)+sizeof(VERTEX3D));
...
void IntersectionTest( SRFDATNM *srfN, SRFDATNM *srfM )
  ...
  R4 lV12, lV13, lV23;  /* lengths of vectors */
  VERTEX3D intP[4];     /* intersection points */
  R4 eps, eps1;  /* rounding factors */
...
IX CylinderRadiusTest( SRFDAT3D *srf, SRFDATNM *srfN, SRFDATNM *srfM,
  IX *possibleObstr, R4 distNM, IX nPossObstr )
/* srf  - data for all surfaces.
 * srfN - data for surface N.
 * srfM - data for surface M.
 * possibleObstr  - list of possible obstructing surfaces (output).
