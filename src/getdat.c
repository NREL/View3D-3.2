...
void GetSrfD( I1 **name, R4 *emit, IX *base, IX *cmbn,
  ...
  n = ReadI2( 0 );              /* base surface number */
  base[ns] = n;
  ...
  n = ReadI2( 0 );              /* combine surface number */
  cmbn[ns] = n;
 ...
 */
  ...
  R4 azm, tilt;     /* azimuth and tilt angles of surface plane */
  R8 sa, ca, st, ct;
  VERTEX3D v[4];
  ...
  while( NxtWord( _string, 1, sizeof(_string) ) != NULL )
    ...
    switch( c )
      ...
      case 'N':
        ...
        xyzLLC.x = ReadR4( 1 );      /* surface geometry data */
        xyzLLC.y = ReadR4( 0 );
        xyzLLC.z = ReadR4( 0 );
        azm = ReadR4( 0 );
        tilt = ReadR4( 0 );
        ...
        if( s == 'R' )               /* rectangle */
          ...
          srf[ns].nv = 4;
          v[0].x = v[1].x = 0.0;
          v[2].x = v[3].x = ReadR4( 0 );   /* width */
          v[0].y = v[3].y = ReadR4( 0 );   /* height */
          v[1].y = v[2].y = 0.0;
          }
        else if( s == 'T' )
          {
        ...
        for( j=0; j<4; j++ )
          ...
          nv++;
          srf[ns].v[j] = xyz+nv;
          srf[ns].v[j]->x = (R4)(xyzLLC.x + v[j].x * ca 
                          - (v[j].y * ct - v[j].z * st ) * sa);
          srf[ns].v[j]->y = (R4)(xyzLLC.y + v[j].x * sa 
                          + (v[j].y * ct - v[j].z * st ) * ca);
          srf[ns].v[j]->z = (R4)(xyzLLC.z + v[j].y * st + v[j].z * ct);
          }
...
void SetPlane( SRFDAT3D *srf )
  ...
  if( srf->nv==4 )               /* quadrilateral */
    ...
    R4 a[5],z[4];  /* temporary values: areas, elevations */
    for( j=0; j<4; j++ )
      VCOPY( (srf->v[j]), (p+j) );
...
void TestSubSrf( SRFDAT3D *srf, const IX *baseSrf, VFCTRL *vfCtrl )
  {
  IX size=(3*sizeof(IX)+2*sizeof(R4)+sizeof(DIRCOS)+sizeof(VERTEX3D));
  IX n;        /* surface number */
