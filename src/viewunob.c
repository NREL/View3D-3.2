...
#if( DEBUG > 0 )
          if( r2 < EPS )
            error( 2, __FILE__, __LINE__, "log(r2) ", FltStr(r2,6), "" );
#endif
...
#if( DEBUG > 0 )
    if( b2 < EPS2 )
      error( 2, __FILE__, __LINE__, "small b2 ", FltStr(b2,6), "" );
#endif
    b = sqrt( b2 );
    binv = 1.0 / b;    /* b > 0.0 */
 ...
 * *flag; return 1 if g=0; else return 0. */
  ...
  if( sxb2 > EPS2*b2 )
    ...
    if( g > EPS2 )
      ...
      error( 3, __FILE__, __LINE__, "Invalid geometry, call George", "" );
    *flag = 0;
    }
  else
    {
...
# if defined __TURBOC__  
  fprintf( _ulog, "  LIadapt: %d (%d) F3 %g F5 %g d %e\n",
    level, _SP, F3, F5, fabs(F5 - F3) );
# elif defined __WATCOMC__
  fprintf( _ulog, "  LIadapt: %d (%d) F3 %g F5 %g d %e\n",
    level, stackavail(), F3, F5, fabs(F5 - F3) );
# else
  fprintf( _ulog, "  LIadapt: %d F3 %g F5 %g d %e\n",
    level, F3, F5, fabs(F5 - F3) );
# endif
  fflush( _ulog );
#endif
...
#if( DEBUG > 0 )
  if( nv1>MAXNV1 )
    error( 2, __FILE__, __LINE__, "NV1 too large ", "" );
#endif
#if( DEBUG > 1 )
...
#if( DEBUG > 0 )
    if( a[i] < EPS )
      error( 2, __FILE__, __LINE__, "small edge (a) ", FltStr(a[i],6), "" );
#endif
    }
...
#if( DEBUG > 0 )
    if( b < EPS )
      error( 2, __FILE__, __LINE__, "small edge (b) ", FltStr(b,6), "" );
#endif
...
void ViewsInit( IX maxDiv, IX init )
  ...
  if( init )
    {
    maxRC1 = MAXNV1;
    _rc1 = Alc_V( 0, maxRC1, sizeof(EDGEDCS), "rc1" );
    maxDV1 = maxDiv - 1;
    _dv1 = Alc_MC( 0, maxRC1, 0, maxDV1, sizeof(EDGEDIV), "dv1" );
    maxRC2 = MAXNVT;
    _rc2 = Alc_V( 0, maxRC2, sizeof(EDGEDCS), "rc2" );
    maxDV2 = maxDiv - 1;
    _dv2 = Alc_MC( 0, maxRC2, 0, maxDV2, sizeof(EDGEDIV), "dv2" );
    }

  else
    {
    Fre_MC( _dv2, 0, maxRC2, 0, maxDV2, sizeof(EDGEDIV), "dv2" );
    Fre_V( _rc2, 0, maxRC2, sizeof(EDGEDCS), "rc2" );
    Fre_MC( _dv1, 0, maxRC1, 0, maxDV1, sizeof(EDGEDIV), "dv1" );
    Fre_V( _rc1, 0, maxRC1, sizeof(EDGEDCS), "rc1" );
  ...
  const R4 _gqx[10] = {    /* Gaussian ordinates */
     0.500000000f, 0.211324865f, 0.788675135f, 0.112701665f, 0.500000000f, 
     0.887298335f, 0.069431844f, 0.330009478f, 0.669990522f, 0.930568156f };
  const R4 _gqw[10] = {    /* Gaussian weights */
     1.000000000f, 0.500000000f, 0.500000000f, 0.277777778f, 0.444444444f,
     0.277777778f, 0.173927423f, 0.326072577f, 0.326072577f, 0.173927423f };
 ...
 */
  ...
  R4 s;    /* distance between vertices */
  IX i, im1, j, n;
  ...
  if( nDiv > 4 )
    {
    error( 2, __FILE__, __LINE__, "nDiv > 4", "" );
    nDiv = 4;
    }
#endif
 ...
 * w    - Gaussian weights */
  ...
  for( i=0; i<nDiv; i++ )
    ...
    for( j=0; j<nDiv; j++,p++,w++ )
      ...
      *w = _gqw[n+i] * _gqw[n+j];     /* incorrect for a */
...
IX SubSrf( const IX nDiv, const IX nv, const VERTEX3D *Sv, const R4 area,
  VERTEX3D *Gpt, R4 *wt )
/* nDiv - division factor
 ...
 * w    - Gaussian weights */
  ...
  static const R4 gx[25][4] = {  /* Gaussian ordinates & weights */
     {0.33333333f, 0.33333333f, 0.33333333f, 1.00000000f },  /* 1-point */

     {0.33333333f, 0.33333333f, 0.33333333f, -0.5625000f },  /* 4-point */
     {0.60000000f, 0.20000000f, 0.20000000f, 0.52083333f },
     {0.20000000f, 0.60000000f, 0.20000000f, 0.52083333f },
     {0.20000000f, 0.20000000f, 0.60000000f, 0.52083333f },

     {0.33333333f, 0.33333333f, 0.33333333f, 0.22500000f },  /* 7-point */
     {0.05971587f, 0.47014206f, 0.47014206f, 0.13239415f },
     {0.47014206f, 0.05971587f, 0.47014206f, 0.13239415f },
     {0.47014206f, 0.47014206f, 0.05971587f, 0.13239415f },
     {0.79742699f, 0.10128651f, 0.10128651f, 0.12593918f },
     {0.10128651f, 0.79742699f, 0.10128651f, 0.12593918f },
     {0.10128651f, 0.10128651f, 0.79742699f, 0.12593918f },
     
     {0.33333333f, 0.33333333f, 0.33333333f, -0.14957004f }, /* 13-point */
     {0.47930807f, 0.26034597f, 0.26034597f, 0.17561526f },
     {0.26034597f, 0.47930807f, 0.26034597f, 0.17561526f },
     {0.26034597f, 0.26034597f, 0.47930807f, 0.17561526f },
     {0.86973979f, 0.06513010f, 0.06513010f, 0.05334724f },
     {0.06513010f, 0.86973979f, 0.06513010f, 0.05334724f },
     {0.06513010f, 0.06513010f, 0.86973979f, 0.05334724f },
     {0.63844419f, 0.31286550f, 0.04869031f, 0.07711376f },
     {0.63844419f, 0.04869031f, 0.31286550f, 0.07711376f },
     {0.31286550f, 0.63844419f, 0.04869031f, 0.07711376f },
     {0.31286550f, 0.04869031f, 0.63844419f, 0.07711376f },
     {0.04869031f, 0.63844419f, 0.31286550f, 0.07711376f },
     {0.04869031f, 0.31286550f, 0.63844419f, 0.07711376f }
